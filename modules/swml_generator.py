...
def generate_swml_response(agent_id, request_body):
    ...
    if enable_record_feature:
        ...
        swml.add_application(
            "main", "record_call", {"stereo": True, "format": enable_record_feature}
        )

    if outbound:
        prompt = AIPrompt.query.filter_by(
            agent_id=agent_id, prompt_type="outbound_prompt"
        ).first()
        post_prompt = AIPrompt.query.filter_by(
            agent_id=agent_id, prompt_type="outbound_post_prompt"
        ).first()
    else:
        prompt = AIPrompt.query.filter_by(
            agent_id=agent_id, prompt_type="prompt"
        ).first()
        post_prompt = AIPrompt.query.filter_by(
            agent_id=agent_id, prompt_type="post_prompt"
        ).first()
    ...
    if post_prompt:
        post_prompt_data = {
            "temperature": (
                post_prompt.temperature if post_prompt.temperature is not None else 0.5
            ),
            "top_p": post_prompt.top_p if post_prompt.top_p is not None else 0.5,
            "text": post_prompt.prompt_text,
        }
        if (
            post_prompt.frequency_penalty is not None
            and post_prompt.frequency_penalty != 0.0
        ):
            post_prompt_data["frequency_penalty"] = post_prompt.frequency_penalty
        if (
            post_prompt.presence_penalty is not None
            and post_prompt.presence_penalty != 0.0
        ):
        ...
        swml.set_aipost_prompt_url(
            {
                ...
                "post_prompt_auth_user": auth_user,
                "post_prompt_auth_password": auth_pass,
            }
        )
    ...
    if auth_user and auth_pass:
        web_hook_url = (
            f"https://{auth_user}:{auth_pass}@{request.host}/onboard/swaig/{agent_id}"
        )
    ...
    if auth_user and auth_pass:
        debug_webhook_url = (
            f"https://{auth_user}:{auth_pass}@{request.host}/debugwebhook/{agent_id}"
        )
    ...
    languages = (
        AILanguage.query.filter_by(agent_id=agent_id)
        .order_by(AILanguage.language_order.asc())
        .all()
    )
    for language in languages:
        ...
        if language.speech_fillers:
            language_data["speech_fillers"] = [
                f.strip() for f in language.speech_fillers.split(",")
            ]
        if language.function_fillers:
            language_data["function_fillers"] = [
                f.strip() for f in language.function_fillers.split(",")
            ]
    ...
    for function in functions:
        ...
        function_args = AIFunctionArgs.query.filter_by(
            function_id=function.id, agent_id=agent_id
        ).all()
        for arg in function_args:
            function_data["parameters"]["properties"][arg.name] = {
                ...
                "type": (
                    arg.type
                    if arg.type
                    in ["integer", "number", "boolean", "string", "array", "object"]
                    else "string"
                ),
                **(
                    {
            ...
            if arg.enum and arg.type == "array":
                function_data["parameters"]["properties"][arg.name]["enum"] = (
                    arg.enum.split(",")
                )
    ...
    enable_internal_fillers = get_feature(agent_id, "ENABLE_INTERNAL_FILLERS")

    if enable_internal_fillers:
        swml.add_aiinternalfillers(
            {
                "get_visual_input": {
                    "en-US": [
        ...
        testimonial_file = get_signalwire_param_by_agent_id(agent_id, "TESTIMONIAL_FILE")
        if testimonial_file:
    ...
    for step, context in context_steps:
        try:
            ...
            if step.valid_steps:
                valid_step_ids = (
                    json.loads(step.valid_steps)
                    if isinstance(step.valid_steps, str)
                    else step.valid_steps
                )
                valid_steps = [
                    s.name
                    for s in AISteps.query.filter(AISteps.id.in_(valid_step_ids)).all()
                ]

            valid_contexts = []
            if step.valid_contexts:
                valid_context_ids = (
                    json.loads(step.valid_contexts)
                    if isinstance(step.valid_contexts, str)
                    else step.valid_contexts
                )
                valid_contexts = [
                    c.context_name
                    for c in AIContext.query.filter(
                        AIContext.id.in_(valid_context_ids)
                    ).all()
                ]
            ...
            if step.functions:
                function_ids = (
                    json.loads(step.functions)
                    if isinstance(step.functions, str)
                    else step.functions
                )
                functions = [
                    f.name
                    for f in AIFunctions.query.filter(
                        AIFunctions.id.in_(function_ids)
                    ).all()
                ]
            ...
            step_dict = {
                k: v for k, v in step_dict.items() if v is not None and v != ""
            }
    ...
    if enable_transfer_feature:
        ...
        transfer.add_application(
            "main",
            "connect",
            {"to": "%{meta_data.table.%{lc:args.target}}", "from": assistant_number},
        )
                    ...
                    "expressions": [
                        {
                            ...
                            "output": {
                                ...
                                "post_process": "true",
                                "action": [
                                    {"SWML": transfer.render(), "transfer": "true"}
                                ],
                            },
                        },
                        {
    ...
    if enable_datasphere_feature:
        ...
        encoded_credentials = base64.b64encode(
            f"{project_id}:{auth_token}".encode()
        ).decode()
